		...
		if (isset($_POST['submit'])){
			...
			if($passOld == $pass){
				...
				if ($resultUpdate == true){ //update thanh công
					header("LOCATION: /www/tour/admincp/?module=ql_users&action=index_user&current=tk&msg=Đổi mật khẩu thành công !");
				} else { //update ko thanh công
					echo "Có lỗi trong quá trình sửa. Vui lòng thử lại!";
				}
			}else{
				header("LOCATION: /www/tour/admincp/?module=ql_users&action=change_pass&current=tk&msg=Mật khẩu chưa đúng");
